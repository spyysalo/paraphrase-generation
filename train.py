...
# Instruction prefix
PREFIX = 'Muotoile seuraava teksti eri sanoin:\n\n'
...
def get_outputs(ref_ids, pred_ids, tokenizer):
    ...
    # remove everything starting at the first remaining eos
    for i in range(len(ref_ids)):
        try:
...
def print_generation(label, model, tokenizer, text=None):
    ...
    if text is None:
        text = PREFIX + 'Luettele seuraavassa tekstissä mainitut henkilönnimet.\n\n'
